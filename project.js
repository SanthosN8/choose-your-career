...
function calculateCareer() {
    let careerMapping = {
        ...
        "entry": "Entry-Level Tech"
    };


    let careerCount = {};
    Object.values(answers).forEach(value => {
        careerCount[value] = (careerCount[value] || 0) + 1;
    });

    let bestCareer = Object.keys(careerCount).reduce((a, b) => 
        careerCount[a] > careerCount[b] ? a : b
    );

    document.getElementById("modalResult").innerHTML = 
        `Your ideal career path is: <strong>${careerMapping[bestCareer]}</strong>`;
        ...
        if (resetInput) {
            ...
            function showCongratsLetters() {
                ...
                for (let i = 0; i < text.length; i++) {
                    ...
                    letter.style.color = ["red", "blue", "green", "orange", "purple"][Math.floor(Math.random() * 5)];
...
function showSuccessPopup() {
    ...
    // // 🎉 Trigger "CONGRATS" confetti
    setTimeout(() => showCongratsLetters(), 700);
